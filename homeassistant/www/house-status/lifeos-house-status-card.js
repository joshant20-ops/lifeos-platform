  ...
  render(){
    ...
    const css=`<style>
      ...
      *{box-sizing:border-box}.app{background:linear-gradient(145deg,#061720,#031018);color:var(--text);border-radius:14px;padding:14px;font-family:var(--paper-font-body1_-_font-family,Arial,sans-serif)}
      ...
      .panel{border:1px solid var(--line);border-radius:10px;background:rgba(6,25,35,.82);padding:12px;margin-top:10px}.heading{font-size:20px;font-weight:750}.sub{color:var(--muted);margin:4px 0 8px}.chartslot{min-height:360px;position:relative;border-top:1px solid rgba(255,255,255,.03);overflow:hidden}.chart{width:100%;height:360px}.gridline{stroke:#47606c;stroke-dasharray:4 5;opacity:.65}.axis{fill:#b8c6ce;font-size:11px}.price{fill:none;stroke:#ffad18;stroke-width:3}.use{fill:none;stroke:#1e9cf0;stroke-width:4}.export{fill:none;stroke:#ef5547;stroke-width:4}.bar-house{fill:#168cff}.bar-batt{fill:#8756d8}.bar-export{fill:#20a86b}.soc{fill:none;stroke:#a9b7c0;stroke-width:2;stroke-dasharray:5 4}.legendrow{display:flex;gap:18px;justify-content:center;flex-wrap:wrap;color:var(--muted);font-size:12px;margin-top:7px}.dot{display:inline-block;width:10px;height:10px;border-radius:50%;margin-right:5px}
    ...
    const svgChart=(flow=false)=>{
      ...
      const cost=pts(report,'domestic_import_cost_gbp',1000), exp=pts(report,'export_earnings_gbp',-1000);
      const all=[...price,...cost,...exp].map(x=>x[1]); let lo=Math.min(0,...all),hi=Math.max(1,...all); const y=v=>pad+ih-(v-lo)/(hi-lo||1)*ih;
      const path=a=>a.map((p,i)=>(i?'L':'M')+p[0].toFixed(1)+' '+y(p[1]).toFixed(1)).join(' ');
      let grid='';for(let i=0;i<5;i++){const yy=pad+i*ih/4;grid+='<line class="gridline" x1="'+pad+'" y1="'+yy+'" x2="'+(W-pad)+'" y2="'+yy+'"/>';}
      if(!flow)return '<svg class="chart" viewBox="0 0 '+W+' '+H+'" preserveAspectRatio="none">'+grid+'<path class="price" d="'+path(price)+'"/><path class="use" d="'+path(cost)+'"/><path class="export" d="'+path(exp)+'"/></svg><div class="legendrow"><span><i class="dot" style="background:#ffad18"></i>Electricity price</span><span><i class="dot" style="background:#1e9cf0"></i>Electricity used cost</span><span><i class="dot" style="background:#ef5547"></i>Export earnings</span></div>';
