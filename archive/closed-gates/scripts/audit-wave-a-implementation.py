    ...
    for k,v in x.items():
      ...
      if isinstance(v,str):
        ...
        if str(k) in ('command','shell_command'):
          try:
            parts=shlex.split(v)
            if parts: a['command_basenames'].add(pathlib.Path(parts[0]).name)
            for part in parts:
              if part.endswith('.py') and part.startswith('/config/'): a['helper_paths'].add(part)
          except Exception: pass
      walk(v,p,a)
  elif isinstance(x,list):
    for v in x[:100]: walk(v,path+['[]'],a)
for f in files:
 p=pathlib.Path(f); a={'exists':p.exists(),'paths':set(),'ids':[],'entity_refs':set(),'command_basenames':set(),'helper_paths':set()}
 if p.exists():
  d=yaml.safe_load(p.read_text()) or {}; walk(d,[],a)
 for k in ('paths','entity_refs','command_basenames','helper_paths'): a[k]=sorted(x for x in a[k] if x)
 out[f]=a
...
obj=json.loads(di.stdout)[0]; mounts=[]
...
probe=r'''
import os,ast,json
roots=['/app','/opt/lifeos-energy']; files=[]; symbols=[]
for root in roots:
  ...
  for fn in fs:
   ...
   if fn.endswith('.py'):
    try:
     ...
     for n in ast.walk(t):
      if isinstance(n,(ast.FunctionDef,ast.AsyncFunctionDef,ast.ClassDef)) and any(k in n.name.lower() for k in ('opportun','negative','event','ledger','dedup','notify','attention')): symbols.append(n.name)
    except Exception: pass
...
report={'schema_version':3,'mode':'read_only_implementation_gap','ha':ha_struct,'ha_mounts':mounts,'energy':energy,'states_emitted':False,'payloads_emitted':False,'secrets_emitted':False,'commands_emitted':False,'mutation_performed':False}
...
for f,a in ha_struct.items():
 ...
 for x in a['helper_paths']: print('HA_HELPER_PATH='+f+'='+x)
