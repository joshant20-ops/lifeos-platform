...
- **HA dashboard rationalisation acceptance scope** — as part of the full HA estate audit, explicitly inventory sidebar dashboards `Homelab`, `LifeOS`, `LifeOS Control`, and `Energy` plus built-in Overview. Map every custom dashboard/view/card to its entity/source, owner, purpose and live usefulness. Identify overlap and historical/dead operational content (including legacy Watchtower/pi3/conflict/playbook records) and distinguish audit history from current alerts. Preserve anything uniquely useful before removal. Preferred outcome, only if evidence supports it: consolidate overlapping Homelab/LifeOS/LifeOS Control operational functions into one authoritative operations dashboard with clear views; keep Energy separate where its purpose remains distinct. Remove/retire redundant sidebar dashboards only after live parity and rollback evidence. Acceptance requires no configuration-error cards, no broken/missing entity references, no stale historical record presented as current work, concise human-readable labels, mobile usability, and live verification after deployment.


## Post-P10 acceptance — whole-system audit and document quality learning

After P5→P10 have each passed sequential real acceptance, run the already-scoped whole-system/HA audit and repair cycle. Reconcile live runtime against canonical GitHub state, fix ordinary defects autonomously through governed deployment, regress accepted capabilities, and retain explicit evidence/rollback.

Then add a permanent **document classification quality loop**. Paperless remains document/metadata authority and OTS-first matching remains first. Randomly sample already-processed documents across labels, including multi-label and low/medium-confidence cases. The private document content and private derived facts stay on Tower/local Ollama. The local reviewer must test whether each assigned label is semantically justified and flag implausible combinations (for example simultaneous house/car/tax) for evidence-based review rather than assuming they are wrong.

External corroboration may use web search only through a privacy boundary: generate a sanitised, non-identifying query locally; reject queries containing names, addresses, registration numbers, account/policy/reference numbers, document text, unique amounts/dates or other personal identifiers; send only the sanitised generic query to the approved web/cloud search path; return public evidence to the local reviewer. Web evidence is advisory and must never expose the source document or become authority over Paperless/accounting records.

Learning must be governed rather than uncontrolled self-training: record each discovered false-positive/false-negative pattern as a local correction candidate with provenance; validate it against a held-out/random sample and existing regression fixtures; promote only demonstrated general rules/config/matching improvements through Git + tests + rollback; re-evaluate affected corpus safely; measure precision/contradiction rate before/after. Never learn a broad rule from one document, never silently rewrite authoritative metadata, and never allow a web result or model assertion alone to create a tax-deductible classification. Tax relevance/deductibility remains governed by `wave-c-tax-evidence-semantics.md`.

Acceptance: random QA is reproducible and privacy-safe; contradictory/implausible label combinations are surfaced; sanitisation has adversarial tests; promoted corrections measurably improve held-out quality without regression; corrections are durable across runs; the system periodically resamples so future classification drift is detected and useful validated learning is applied.
