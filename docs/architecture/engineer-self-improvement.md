...
The protected control plane must first create a root-owned, non-group/world-writable approval record at `/var/lib/lifeos-control/engineer-deploy-approvals/<job-id>.json`. That record binds the job to the exact published `main` commit, both allow-listed source hashes, an independent-verifier PASS evidence ID, and a protected-policy PASS evidence ID. The Engineer cannot create this evidence. Missing, malformed, writable, refused, future-dated, more than 24 hours old, or mismatched evidence fails closed.

The broker independently requires a clean canonical `/home/joshan/lifeos-platform` checkout whose `HEAD`, local `main`, and `origin/main` are the approved commit. It rejects symlinked or unexpectedly owned sources and destinations, pins each source as verified bytes to close pathname races, verifies both source hashes and Python syntax, and can only map:
...
The transaction takes a non-blocking exclusive lock, rejects replayed job IDs before source or destination access, backs up both installed files, atomically replaces each file, restarts only `lifeos-autonomous-agent.service` and `lifeos-engineer.service`, and waits for the fixed ports 8790 and 8793 health endpoints. Any installation, restart, or health failure atomically restores both backups, restarts both services, and verifies rollback health. A root-owned, durable, create-only audit record preserves commit, hashes, backup location, deployment result, and rollback result; an existing record can never be overwritten.
