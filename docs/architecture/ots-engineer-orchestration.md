...
Rundeck Community is the target orchestration and audit service, superseding the earlier Semaphore design before either product was deployed from this repository. A pinned shadow definition now exists under `orchestration/rundeck`; this does not claim that a live-host deployment or inventory has run.
...
The target path is Engineer -> canonical `lifeos-platform` source -> bounded LifeOS policy/client -> Rundeck -> version-controlled Ansible -> #26 transaction controller -> bounded submission bridge/root broker -> Pi5, Docker, and Home Assistant. Rundeck receives no generic sudo, arbitrary root shell, caller-selected root path, or wildcard service authority.
...
| Rundeck Community | Scheduling, queueing, retries, task state, logs, history, and operator UI |
...
| Earlier Semaphore design | REPLACE | Issue #23 explicitly selects Rundeck; no repository deployment existed to migrate |
| Filesystem queue promotion | RETIRE_AFTER_SHADOW | Rundeck owns queueing |
| Custom retry scheduler | RETIRE_AFTER_SHADOW | Rundeck owns retries/history |
| Custom execution state and UI | RETIRE_AFTER_SHADOW | Rundeck owns state, logs, and UI |
...
2. Add and runtime-validate the LAN-only Rundeck shadow deployment in `orchestration/rundeck`, with persistent storage, secrets supplied outside Git, health checks, restart policy, resource limits, and backup/restore procedures.
