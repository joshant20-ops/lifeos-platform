...
Activation is intentionally a human-controlled privilege change: create the dedicated service account and socket group, grant that account write/search access only to the four directories (prefer named ACLs), write the Engineer UID to `/etc/lifeos-control/control-job-submit.conf`, install the unit/helper, and enable the socket. The activation proof must be invoked through the governed runtime launcher staged outside Git under `/var/lib/lifeos-agent/runtime_jobs`. It invokes the reviewed, timeout-bounded activator, verifies denial and acceptance behavior, and only then continues the previously approved job through the normal publisher queue.
