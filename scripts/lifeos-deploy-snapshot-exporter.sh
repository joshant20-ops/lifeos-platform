...
SERVICE_DEST=/etc/systemd/system/lifeos-snapshots-export.service
TIMER_DEST=/etc/systemd/system/lifeos-snapshots-export.timer
...
for file in "$SERVICE_SOURCE" "$TIMER_SOURCE" "$SNAPSHOT_SCRIPT"; do
  [[ -f "$file" && ! -L "$file" ]] || { echo 'SNAPSHOT_EXPORT_DEPLOY=FAIL'; echo "REASON=canonical_source_missing:$file"; exit 1; }
done
