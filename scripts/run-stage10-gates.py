...
FILES=['governor/contracts/engineering-work.schema.json','governor/contracts/examples/stage10-smoke.json','scripts/validate-engineering-work-contract.py','tests/test_engineering_work_contract.py']
...
def fail(gate,detail=''):
 ...
 print('===== DIAGNOSTICS =====')
 for label,cmd in [('git',['git','status','--short','--branch']),('recent commits',['git','log','--oneline','--decorate','-8']),('contract diff',['git','diff','--']+FILES),('containers',['docker','ps','--format','{{.Names}}\t{{.Status}}'])]: show(label,cp(cmd))
 raise SystemExit(1)
def gate(name,cmd,needle=None):
 r=cp(cmd)
 if r.returncode or (needle and needle not in r.stdout): fail(name,(r.stdout+r.stderr).strip())
 print(f'PASS: {name}')
# Stage 10 artifacts are repository-native. Never rewrite tracked files from the old feature branch.
missing=[f for f in FILES if not (REPO/f).is_file()]
if missing: fail('contract-source','missing tracked artifacts:\n'+'\n'.join(missing))
...
if r.returncode: fail('working-tree-safety',r.stderr.strip())
if r.stdout.strip(): fail('working-tree-safety','repository is not clean:\n'+r.stdout.strip())
...
print('NEXT=Stage 10 end-to-end engineering workflow proof')
