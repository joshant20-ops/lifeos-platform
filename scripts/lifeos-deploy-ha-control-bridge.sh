...
tower=json.loads(sys.argv[2])
if tower.get('state') not in {'OFF','POWERED_INACCESSIBLE','ACCESSIBLE','UNKNOWN'}:
    raise SystemExit('invalid_tower_state=' + str(tower.get('state')))
...
print('TOWER_COLOUR=' + tower.get('colour','gray'))
PY
...
ents=doc.get('data',{}).get('entities',[])
rx=re.compile(r'(tower|desktop|computer|workstation)',re.I)
for e in ents:
    text=' '.join(str(e.get(k) or '') for k in ('entity_id','name','original_name','platform','unique_id'))
    if rx.search(text):
        print('TOWER_HA_CANDIDATE=' + str(e.get('entity_id')) + ' platform=' + str(e.get('platform')) + ' name=' + str(e.get('name') or e.get('original_name') or ''))
PY
