...
# while /config still contains an older running core.
iv=$(docker exec predbat sh -lc "cd /config && python3 -c 'import predbat; print(predbat.THIS_VERSION)'" 2>/dev/null | tail -1 || true)
