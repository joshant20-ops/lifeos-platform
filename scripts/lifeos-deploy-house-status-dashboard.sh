...
HAD_DASH=0; HAD_ASSET=0; MUTATED=0
fail(){ echo "HOUSE_STATUS_DEPLOY=FAIL"; echo "ERROR=$*"; exit 1; }
rollback(){ rc=$?; if [[ "$MUTATED" -eq 1 && "$rc" -ne 0 ]]; then echo "HOUSE_STATUS_ROLLBACK=START"; if [[ "$HAD_DASH" -eq 1 ]]; then cp -a "$BACKUP_DIR/dashboard" "$DASH_TARGET"; else rm -f "$DASH_TARGET"; fi; cp -a "$BACKUP_DIR/registry" "$REGISTRY"; if [[ "$HAD_ASSET" -eq 1 ]]; then cp -a "$BACKUP_DIR/asset" "$ASSET_TARGET"; else rm -f "$ASSET_TARGET"; fi; docker restart homeassistant >/dev/null || true; echo "HOUSE_STATUS_ROLLBACK=COMPLETE"; fi; exit "$rc"; }
trap rollback EXIT
[[ ${EUID:-$(id -u)} -eq 0 ]] || fail "must_run_as_root"
[[ -f "$DEPLOYER" && -f "$VERIFIER" && -f "$SOURCE_ASSET" ]] || fail "repository_sources_missing"
...
install -o root -g root -m 0644 "$SOURCE_ASSET" "$ASSET_TARGET"; MUTATED=1
python3 "$DEPLOYER"
