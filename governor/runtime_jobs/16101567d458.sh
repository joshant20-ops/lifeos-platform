...
readonly JOB_ID=powerdown-telemetry-16101567d458
...
fail() {
  printf 'FAIL=%s\n' "$1" >&2
  printf '%s\n' 'RESULT=RETRY' 'TESTS=bounded deployment or live acceptance failed' \
...
readonly HEAD=$(git -C "$PLATFORM" rev-parse HEAD)
...
trap 'rc=$?; if ((rc)); then rollback || printf "%s\n" DEPLOYMENT_ROLLBACK=FAIL; fi; exit $rc' EXIT
...
case "$LIVE_SHA" in
  ...
  "$PRE_FIX_SHA"|"$FIX_ONLY_SHA")
    ...
    BACKUP="$BACKUP_DIR/${LIFEOS_JOB_ID}-${LIVE_SHA}.bak"
...
d={'schema_version':1,'job_id':'powerdown-telemetry-16101567d458','target':'pi5','job_type':'change',
   'script':'jobs/root-scripts/powerdown-telemetry-16101567d458.sh','script_sha256':digest,
   'timeout_seconds':300,'created_by':'lifeos-cloud-builder-16101567d458',
...
printf '%s\n' \
  ...
  'BARRIER=none' \
  'NEXT_AUTONOMOUS_ACTION=post-event assurance job verifies recorded event release and restoration after 19:00' \
  ...
  'NEXT_RUNTIME_CHECK=after 19:00 run the committed Power Down V52 post-event assurance job against event-evidence-v52.jsonl'
