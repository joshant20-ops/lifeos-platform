...
wakes Tower through the existing AI broker path when required, and proxies only
the Qwen2.5-Coder model to Tower Ollama. No inference runs on the Pi.
"""
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path=="/v1/models": return self.sendj(200,{"object":"list","data":[{"id":MODEL,"object":"model","owned_by":"lifeos"}]})
    ...
    def do_POST(self):
        ...
        try:
            ...
            if requested not in {MODEL,"openai/"+MODEL}: raise ValueError("model not allowed")
            payload["model"]=MODEL
            ensure_tower()
